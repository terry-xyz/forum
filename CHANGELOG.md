...
- *(cmd/seed)* add deterministic fake forum data seeding
- *(handlers)* add filtered views for posts created or liked by the current user

### Fixed

- *(handlers/auth.go)* store and verify bcrypt password hashes for login

### Documentation

- *(comments)* rewrite and complete code comments for handlers, database helpers, models, and seed tests
